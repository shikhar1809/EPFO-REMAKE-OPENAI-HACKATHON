    ...
    <motion.div initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} className='flex-1 flex flex-col bg-slate-50'>
      ...
      <div className='p-6 flex-1 overflow-y-auto'>
        <div className='bg-white rounded-3xl p-6 shadow-sm mb-6 text-center border border-slate-100'>
          ...
          <h2 className='text-4xl font-bold text-slate-800 mb-6'>â‚¹ 3,42,500</h2>
          ...
          <div className='flex justify-between text-xs text-slate-600 mt-4'>
            <div className='text-left'>
              ...
              <p className='font-semibold text-slate-800 mt-1'>â‚¹ 1,50,000</p>
            </div>
          ...
          {passbook.map((entry) => (
            <div key={entry.id} className='bg-white p-4 rounded-2xl shadow-sm border border-slate-100'>
              <div className='flex justify-between mb-2'>
                ...
                <span className='font-semibold text-green-600'>+â‚¹ {entry.employeeShare + entry.employerShare}</span>
              </div>
              <div className='text-xs text-slate-500 flex justify-between'>
                <span>You: â‚¹{entry.employeeShare}</span>
                <span>Employer: â‚¹{entry.employerShare}</span>
                <span>Pension: â‚¹{entry.pensionShare}</span>
              </div>
            </div>
          ))}
        </div>
